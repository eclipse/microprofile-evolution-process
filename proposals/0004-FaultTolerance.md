...
* Event Hooks: Since this approach to fault tolerance involves handing execution over to some foreign code, it's very useful to be able to learn when executions are taking place (before/after) and under what circumstances (`onRetry`, `onFailedAttempt`, etc). 
...
The design is based on CDI programming model. An instance of `RetryPolicy`, `CircuitBreaker`, BulkHead, `Execution` or a fallback can be injected to the clients and then they can be configured as follows.
...
When `TimeoutException` was received, delay 2 seconds and then retry up to 2 more times.

```
Connection connection = execution.with(rp).withFallback(null).get(this::connect);
```

The above suppresses the `TimeoutException` and provides a default result.
